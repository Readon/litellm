...
from litellm.litellm_core_utils.streaming_handler import GenericStreamingChunkHandler
...
class HostedVLLMChatCompletionStreamingHandler(GenericStreamingChunkHandler):
    """
    Streaming handler for hosted_vllm provider.
    Maps delta.reasoning to delta.reasoning_content to match LiteLLM's expected format.
    Similar to GroqChatCompletionStreamingHandler.
    """
    
    def map_response(self, model_response, **kwargs):
        """Map reasoning field to reasoning_content for streaming responses."""
        if isinstance(model_response, ModelResponse):
            for choice in model_response.choices:
                if hasattr(choice, 'delta') and choice.delta:
                    delta = choice.delta
                    # Check for reasoning field (from vLLM/SGLang) and map to reasoning_content
                    if hasattr(delta, 'reasoning') and delta.reasoning is not None:
                        delta.reasoning_content = delta.reasoning
                    # Also check in dict form
                    elif isinstance(delta, dict) and 'reasoning' in delta and delta['reasoning']:
                        delta['reasoning_content'] = delta['reasoning']
